...
template<typename PixelType>
void ofPixels_<PixelType>::setFromAlignedPixels(const PixelType * newPixels, size_t width, size_t height, ofPixelFormat _pixelFormat, std::vector<int> strides) {
	std::vector<size_t> stridesU(strides.size());
	std::transform(strides.begin(), strides.end(), stridesU.begin(), [](int i){return size_t(i);});
	setFromAlignedPixels(newPixels, width, height, _pixelFormat, stridesU);
}
